...
tags:                             # optional — named context variants
  experimental:
    mappings:                     # same shape as root mappings
      src/api:                    # overrides the root mapping for this target
        context_source: ./agents/backend/AGENTS.exp.md
      src/docs:                   # adds a target not present in root
...
### Tags (context variants)

- `tags` is optional. Each named tag has its own `mappings` record (identical
  shape + validation as root mappings, including the file-existence and
  traversal guards). A tag with zero mappings → exit 2.
- **Effective mappings** for a selected tag = root mappings with the tag's
  mappings layered on top: a tag mapping **overrides** a matching root target and
  **adds** any target not in root (root ∪ tag, tag wins on conflict).
- `--tag <name>` on `add`/`update` selects a tag; an unknown name → exit 2
  listing the available tags. With no `--tag`, only the root mappings apply.
- The chosen tag is recorded per lock entry (`tag` field) so `install` reproduces
  it and `update` re-resolves the same variant. Re-running `add --tag <other>`
  (or `update --tag <other>`) **switches** the context for those targets.
...
| `subpath` | string \| _(absent)_ | Optional. Contexts root inside the repo when the source was added via a `/tree/<ref>/<subpath>` URL; omitted entirely (not `null`) for root sources. `install`/`update` re-apply it so the cache layout and `contextPath` reproduce exactly. |
| `contextPath` | string | Path of the profile file inside the contexts repo (relative to the subpath when set), POSIX separators. |
